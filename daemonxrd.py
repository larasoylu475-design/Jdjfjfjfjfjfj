...
# ---------------- BOT HANDLERS ---------------- #
# Define your variables with proper newlines
welcome_text = (
...
def start_handler(message):
    photo_url = "https://i.ibb.co/wkXVHH8/20251225-174620.jpg"
    
    # Create Buttons
    markup = types.InlineKeyboardMarkup(row_width=2)
    btn_help = types.InlineKeyboardButton("Help 📖", callback_data="help_cmd")
    btn_ping = types.InlineKeyboardButton("Ping 📶", callback_data="ping_cmd")
    markup.add(btn_help, btn_ping)

    # Send Photo + Message + Buttons
    bot.send_photo(
        message.chat.id, 
        photo_url, 
...
def callback_listener(call):
    if call.data == "help_cmd":
        help_info = (
            "📖 *Manual Instructions*\n\n"
            "• Use the tool to start reporting.\n"
            "• Connect multiple sessions safely.\n"
            "• System status: **Active 2027**\n\n"
            "👤 *Dev:* `daemonxrd /oi`"
        )
        bot.answer_callback_query(call.id)
        bot.send_message(call.message.chat.id, help_info, parse_mode="Markdown")

    elif call.data == "ping_cmd":
        start_time = time.time()
        bot.answer_callback_query(call.id, "Checking speed...")
        end_time = time.time()
